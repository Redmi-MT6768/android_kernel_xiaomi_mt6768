...
#include "s5k5e9_sunny_mipi_raw_Sensor.h"

/****************************Modify following Strings for debug****************************/
#define PFX "s5k5e9_sunny_camera_sensor"
#define LOG_1 LOG_INF("s5k5e9_sunny,MIPI 2LANE\n")
...
static struct imgsensor_info_struct imgsensor_info = { 
    .sensor_id = S5K5E9_SUNNY_SENSOR_ID,
...
{
    LOG_INF("s5k5e9_sunny %s:%d E", __func__, __LINE__);

    // +++++++++++++++++++++++++++//                                                               
    // Reset for operation     
    //Streaming off
    //S5K5E9_SUNNY_EVT0_Reference_setfile_v0.3b_180308
    write_cmos_sensor(0x0100, 0x00); 
    ...
    LOG_INF("s5k5e9_sunny %s:%d X", __func__, __LINE__);
}	/*	sensor_init  */
             ...
             if(0x01 == vendor_id){
                 *sensor_id = return_sensor_id() + 1;
                 if (*sensor_id == imgsensor_info.sensor_id) {
                         pr_info("s5k5e9 sunny i2c write id: 0x%x, sensor id: 0x%x vendor_id: 0x%x\n",
                                                    imgsensor.i2c_write_id, *sensor_id, vendor_id);
                         return ERROR_NONE;
                 }else{
                        pr_err("s5k5e9 sunny check id fail i2c write id: 0x%x, sensor id: 0x%x vendor_id: 0x%x\n",
                                                imgsensor.i2c_write_id, *sensor_id, vendor_id);
...
UINT32 S5K5E9_SUNNY_MIPI_RAW_SensorInit(struct SENSOR_FUNCTION_STRUCT **pfFunc)
{
    /* To Do : Check Sensor status here */
    if (pfFunc!=NULL)
        *pfFunc=&sensor_func;
    return ERROR_NONE;
}	/*	S5K5E9_SUNNY_MIPI_RAW_SensorInit	*/
