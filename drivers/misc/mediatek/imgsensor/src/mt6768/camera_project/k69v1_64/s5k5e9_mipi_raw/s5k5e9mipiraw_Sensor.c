            ...
            if(0x07 == vendor_id){
                *sensor_id = return_sensor_id();
                if (*sensor_id == imgsensor_info.sensor_id) {
                         pr_info("s5k5e9_ofilm i2c write id: 0x%x, sensor id: 0x%x vendor_id: 0x%x\n",
                                                    imgsensor.i2c_write_id, *sensor_id, vendor_id);
                         return ERROR_NONE;
                    }else{
                        pr_err("ov02a10_ofilm check id fail i2c write id: 0x%x, sensor id: 0x%x vendor_id: 0x%x\n",
                                                imgsensor.i2c_write_id, *sensor_id, vendor_id);
